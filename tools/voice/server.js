...
function createServer(store,worker,token){
  return http.createServer(async(req,res)=>{
    ...
    const status=()=>({...store.status(),busy:worker.busy,error:worker.error});
    if(req.method==='GET'&&req.url==='/api/voice')return send(200,status());
    if(req.method!=='POST'||!['/api/voice/configure','/api/voice/check'].includes(req.url))return send(404,{error:'Неизвестный запрос'});
    try{
...
if(require.main===module){
  ...
  const worker=new Worker(store);const server=createServer(store,worker,process.env.VOICE_SERVICE_TOKEN||'');
  ...
  for(const signal of ['SIGINT','SIGTERM'])process.on(signal,()=>{worker.stopped=true;store.data.config.enabled&&store.save();worker.pollController?.abort();server.close();setTimeout(()=>process.exit(0),1000).unref();});
}
