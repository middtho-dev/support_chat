...
class Store {
  constructor(dir,key){
    ...
    for(const field of ['botToken','openaiKey']){const v=this.data.config[field];if(v)this.data.config[field]=this.decrypt(v);}
  ...
  save(){const data={...this.data,config:{...this.data.config}};for(const f of ['botToken','openaiKey'])data.config[f]=data.config[f]?this.encrypt(data.config[f]):'';fs.writeFileSync(this.file+'.tmp',JSON.stringify(data),{mode:0o600});fs.renameSync(this.file+'.tmp',this.file);}
  status(){const {botToken,openaiKey,...config}=this.data.config;return {config,hasBotToken:!!botToken,hasOpenaiKey:!!openaiKey,connections:Object.values(this.data.connections).map(c=>({id:c.id,ownerId:c.user?.id,enabled:c.is_enabled,canReply:!!c.rights?.can_reply,canDelete:!!c.rights?.can_delete_all_messages})),jobs:this.data.jobs.slice(-30).reverse().map(({id,status,chatId,messageId,seconds,error,created})=>({id,status,chatId,messageId,seconds,error,created})),todaySeconds:this.data.quota[new Date().toISOString().slice(0,10)]||0};}
}
