...
class Store {
  constructor(dir,key){
    ...
    delete this.data.config.richCompact;
    for(const j of this.data.jobs)if(j.status==='sending'||j.progressSending) {j.status='uncertain';j.error='Отправка могла завершиться перед перезапуском. Проверьте чат; автоматический повтор отключён.';}
    this.save();
  }
