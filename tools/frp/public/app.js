...
(() => {
  ...
  panel.innerHTML = `<div class="frp-wrap">
    ...
    </details><h3>Подключённые устройства <span id="frp-client-count"></span></h3>
  ...
  function clientProxies(client) { return (current?.devices || []).filter(d => client.clientID && d.clientID === client.clientID && d.user === client.user); }
  function renderDevices() {
    ...
    $('frp-clients').innerHTML = clients.map(c => `<tr><td>${link(window.FrpLinks.primaryProxy(clientProxies(c)), c.user || c.hostname || c.clientID || c.key)}</td><td>${esc(c.ip || '—')}</td><td>${c.stale ? 'Нет данных' : c.online ? '🟢 Онлайн' : 'Отключено'}</td><td>${clientProxies(c).map(d => link(d, `${d.port ?? '—'}/${d.type}`)).join(', ') || '—'}</td></tr>`).join('') || '<tr><td colspan="4">Устройства ещё не подключались</td></tr>';
    ...
    $('frp-devices').innerHTML = devices.filter(d => `${d.name} ${d.port}`.toLowerCase().includes(search))
      ...
      .map(d => `<tr><td>${link(d, d.name)}</td><td>${d.stale ? 'Нет данных' : d.online ? '🟢 Онлайн' : 'Отключено'}</td><td>${esc(d.type.toUpperCase())}</td><td>${esc(d.port ?? '—')}</td><td>${d.online ? esc(d.connections) : '—'}</td><td>${d.lastSeen ? esc(new Date(d.lastSeen).toLocaleString('ru-RU')) : '—'}</td></tr>`).join('') || '<tr><td colspan="6">Устройства не найдены. Подключите frpc с конфигурацией ниже.</td></tr>';
  }
  ...
  window.FrpPanel = {
    open: load,
    reset: () => { current = null; $('frp-config').textContent = ''; $('frp-clients').textContent = ''; $('frp-devices').textContent = ''; $('frp-status').textContent = 'Загрузка…'; }
  };
