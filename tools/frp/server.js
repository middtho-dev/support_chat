...
function createServer(manager, token) {
  ...
  return http.createServer(async (req, res) => {
    ...
    try {
      ...
      const action = url.pathname.match(/^\/api\/admin\/frp\/(install|start|stop|configure|issue-firmware|generate-installer|release-installer|revoke-installer|update-device|delete-device)$/)?.[1];
