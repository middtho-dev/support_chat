...
window.createWorkspaceAnnouncements=function(options){
 ...
 return {update:function(item,token,id,enabled){
  ...
  var card=node('section','wa-notice-card'),brand=node('div','wa-notice-brand'),logo=node('img','');logo.src=options.logo;logo.alt='KV9RU';brand.appendChild(logo);brand.appendChild(node('span','','ОБЪЯВЛЕНИЕ'));
