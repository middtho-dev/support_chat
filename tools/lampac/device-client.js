...
function start(){
 ...
 var playback=window.createWorkspacePlayback?window.createWorkspacePlayback({token:function(){return state.token;},request:request,access:access}):null;
 ...
 function poll(){if(state.paused||state.revoked)return;if(!state.token){enroll();return;}if(busy)return;var token=state.token,payload={token:token,applied:state.applied||0,snapshot:{}};try{payload.snapshot=snapshot();payload.controls=window.workspaceUIControls?window.workspaceUIControls.inventory():[];}catch(e){}busy=true;request('poll',payload,function(status,data){busy=false;if(token!==state.token||state.paused)return;if(status===403){if(announcements)announcements.update(null,token,state.id,false);access(false);if(data.code==='device_revoked'){state={applied:0,overrides:{},enabled:false};save();enroll();}return;}if(status!==200)return;
