...
test('unnamed Lampa action buttons are controlled separately and native visibility survives polling',()=>{
 ...
 vm.runInNewContext(source,context);const api=context.workspaceUIControls,items=api.inventory().filter(x=>x.group==='Пункты: Хранилище');assert.equal(items.length,2);
