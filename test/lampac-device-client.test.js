...
test('device plugin applies scoped preferences and ignores replies after local disconnect',()=>{
  ...
  assert.equal(values.get('workspace_device_access').paused,true);assert.equal(values.get('workspace_device_access').token,'device-token');assert.equal(values.get('internal_torrclient'),'true');
});
