...
window.mountLampacAdvanced = function ({container, request, operation, toggle, publicUrl}) {
  ...
  function renderPlayback(data){
    ...
    panes.playback.innerHTML=`<p class="wk-note">Проверено ${esc(date(data.serverTime))}. Без сигнала более 90 секунд статус просмотра не подтверждён. Внешний плеер может не передавать данные. Скорость TorrServer относится ко всей раздаче, буфер — к этому плееру. Записи без сигнала удаляются автоматически через 5 минут. Завершённые и неподтверждённые записи можно удалить вручную.${data.torrentsAvailable?'':' Скорость TorrServer сейчас недоступна.'}</p><div class="wk-actions"><button class="ghost" data-clear-sessions ${records.some(s=>!s.fresh||s.state==='ended')?'':'disabled'}>Очистить неактивные</button></div><div class="lc-list lc-record-list">${records.map((s,index)=>`<article class="card lc-session">${posterMarkup(s.poster)}<div class="lc-session-content"><div class="lc-session-device"><span>${esc(s.name)}</span> <small class="wk-badge">${esc(!s.enabled?'Доступ отключён':s.fresh?playbackStates[s.state]:'Статус не подтверждён')}</small></div><h3 class="lc-session-title">${esc(s.title||'Название не передано')}</h3><dl class="wk-metadata">${row('Источник',(s.hash?'Торрент · ':'')+(s.source||'Не передан'))}${row('Способ',({'browser':'Встроенный · браузер','browser-hls':'Встроенный · HLS','native':'Встроенный · система устройства'})[s.method]||'Не определён')}${row('Позиция',seconds(s.position)+' / '+seconds(s.duration))}${row('Буфер плеера',s.fresh&&s.buffer!=null?s.buffer+' с':'Недоступно')}${row('Скорость раздачи',s.fresh&&s.downloadSpeed!=null?bytes(s.downloadSpeed)+'/с':'Недоступно')}${row('Последний сигнал',date(s.updated))}${row('Последняя ошибка',playbackErrors[s.error]||'Не зарегистрирована')}</dl>${!s.fresh||s.state==='ended'?`<div class="wk-actions"><button class="ghost" data-remove-session="${index}">Удалить запись</button></div>`:''}</div></article>`).join('')||empty('Активных просмотров нет.')}</div>`;
