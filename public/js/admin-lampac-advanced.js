...
window.mountLampacAdvanced = function ({container, request, operation, toggle, publicUrl}) {
  ...
  function preferences(fields, values, snapshot, global = false, shared = {}) {
    ...
    const renderGroup=([group, items]) => `<details class="lc-pref-group"><summary>${esc(group)} <small>${items.length} · задано ${items.filter(f=>Object.hasOwn(values,f.key)).length}</small></summary>${items.map(f => {
    ...
    return [...families].map(([family,entries],i)=>`<details class="lc-pref-family" ${i===0?'open':''}><summary>${esc(family)} <small>${entries.reduce((n,e)=>n+e[1].length,0)} пунктов</small></summary><div class="lc-pref-family-body">${entries.map(renderGroup).join('')}</div></details>`).join('');
  }
  function bindPreferences(form) {
    form.querySelectorAll('[data-managed]').forEach(el=>el.onchange=()=>{el.closest('[data-key]').querySelector('[data-value]').disabled=!el.checked;const group=el.closest('.lc-pref-group');group.querySelector('summary small').textContent=group.querySelectorAll('[data-key]').length+' · задано '+group.querySelectorAll('[data-managed]:checked').length;});
