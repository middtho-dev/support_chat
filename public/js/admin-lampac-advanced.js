...
window.mountLampacAdvanced = function ({container, request, operation, toggle, publicUrl}) {
  ...
  function renderClients(data) {
    ...
    panes.clients.innerHTML = `<details class="card lc-info"><summary>Учёт и блокировка IP</summary><p>Последние ${data.limit || 500} записей · история ${data.retentionDays || 7} дней.</p><details><summary>Учёт подключений и блокировка</summary><p>IP + браузер — наблюдаемое подключение, а не постоянный ID устройства. Несколько устройств за одним роутером могут иметь общий IP. «Недавно» означает запрос за последние 2 минуты, а не подтверждённый просмотр.</p><p>Блокировка закрывает новые запросы к ${esc(publicUrl)} для всего IP. Уже открытый поток может продолжить работу; при необходимости сбросьте его во вкладке «Торренты». Прямые подключения в обход Caddy здесь не учитываются.</p></details><form class="voice-actions" id="lc-block-form"><label class="voice-field">IP для блокировки<input id="lc-block-ip" required maxlength="45" placeholder="IPv4 или IPv6" autocomplete="off"></label><button class="danger" type="submit">Заблокировать IP</button></form><div id="lc-blocked">${blocked.map((ip, i) => `<p><code>${esc(ip)}</code> <button class="ghost" data-unblock="${i}">Разблокировать</button></p>`).join('')}</div></details><div class="lc-list lc-record-list">${list.map((c, i) => `<details class="card lc-record"><summary><span>${esc(c.ip)}</span><small>${blocked.includes(c.ip) ? 'Заблокирован' : Date.now() / 1000 - c.last < 120 ? 'Недавно' : 'Неактивен'}</small></summary><div class="lc-record-body"><p>${esc(c.ua || 'Браузер не передал сведения')}</p><p>Последний запрос: ${esc(date(c.last))} · ${esc(c.route)}<br>Первый запрос: ${esc(date(c.first))} · обращений: ${esc(c.requests)}</p>${c.torrent ? `<p>Последний хеш торрента: <code>${esc(c.torrent)}</code></p>` : ''}<button class="${blocked.includes(c.ip) ? 'ghost' : 'danger'}" data-client-block="${i}">${blocked.includes(c.ip) ? 'Разблокировать' : 'Заблокировать IP'}</button></div></details>`).join('') || empty('Пока нет обращений. Откройте Lampa или запустите видео через публичный адрес.')}</div>`;
