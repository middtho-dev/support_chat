...
window.mountLampacAdvanced = function ({container, request, operation, toggle, publicUrl}) {
  ...
  const tabs = [['overview', 'Сервер'], ['torrents', 'Торренты'], ['clients', 'Подключения'], ['devices', 'Устройства'], ['settings', 'Источники и плагины'], ['client', 'Оформление Lampa']];
  const panes = {};
  ...
  function renderDevices(data) {
    ...
    panes.devices.querySelectorAll('[data-pref-search]').forEach(input=>input.oninput=()=>input.form.querySelectorAll('[data-pref-label]').forEach(label=>label.hidden=!label.dataset.prefLabel.includes(input.value.toLowerCase().trim())));
    panes.devices.querySelectorAll('[data-device-form]').forEach(form=>form.onsubmit=async e=>{
      ...
      const values=Object.fromEntries([...form.querySelectorAll('[data-device-pref]')].filter(el=>el.value&&el.value!=='__inherit').map(el=>[el.dataset.devicePref,el.value]));
      const inherit=[...form.querySelectorAll('[data-device-pref]')].filter(el=>el.value==='__inherit').map(el=>el.dataset.devicePref);
