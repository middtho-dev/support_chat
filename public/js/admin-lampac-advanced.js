...
window.mountLampacAdvanced = function ({container, request, operation, toggle, publicUrl}) {
  ...
  for (const [key, label] of tabs) {
    ...
    button.onclick = () => {
      ...
      const descriptions = {
        ...
        playback:'Текущие сеансы и последняя ошибка. Обновление каждые 5 секунд; данные Lampa и активные потоки TorrServer.',
  ...
  const playbackStates={idle:'Lampa открыта',loading:'Загрузка',playing:'Смотрит',paused:'Пауза',buffering:'Буферизация',ended:'Просмотр завершён',error:'Ошибка плеера',external:'Внешний плеер'};
  ...
  function presence(device,sessions,now){
    ...
    const current=sessions.find(s=>s.device===device.id&&s.fresh&&['playing','buffering','loading','paused','external'].includes(s.state));
    return current?(current.streamActive?'Онлайн · внешний поток':playbackStates[current.state]):now-device.last<90?'На связи':'Нет свежего сигнала';
  }
  function updatePresence(data){
    ...
    panes.devices.querySelectorAll('[data-device-film]').forEach(node=>{const current=data.sessions.find(s=>s.device===node.dataset.deviceFilm&&s.fresh&&s.enabled);node.textContent=current?.title||'';node.parentElement.hidden=!node.textContent;});
  }
  ...
  function renderPlayback(data){
    ...
    panes.playback.innerHTML=`<div class="wk-note"><p>Проверено ${esc(date(data.serverTime))}. Данные Lampa и потоки TorrServer.</p><details><summary>Как определяется просмотр</summary><p>Без сигнала более 90 секунд статус просмотра не подтверждён. Запуск внешнего плеера сообщает Lampa. Пока она в фоне, активный читатель TorrServer подтверждает отдачу видео по hash; позиция и пауза недоступны. Если hash используется несколькими устройствами, поток не приписывается одному из них. Для определения устройства после обновления один раз перезапустите Lampa. Скорость TorrServer относится ко всей раздаче, буфер — к этому плееру. Обычные записи без сигнала исчезают через 5 минут; связь с внешним плеером сохраняется до 12 часов для проверки серверного потока. Завершённые и неподтверждённые записи можно удалить вручную.${data.torrentsAvailable?'':' Скорость TorrServer сейчас недоступна.'}</p></details></div><div class="wk-actions"><button class="ghost" data-clear-sessions ${records.some(s=>!s.fresh||s.state==='ended')?'':'disabled'}>Очистить неактивные</button></div><div class="lc-list lc-record-list">${records.map((s,index)=>`<article class="card lc-session">${posterMarkup(s.poster)}<div class="lc-session-content"><div class="lc-session-device"><span>${esc(s.name)}</span> <small class="wk-badge">${esc(!s.enabled?'Доступ отключён':s.streamActive?(s.device?'Внешний поток активен':'Поток активен'):s.fresh?playbackStates[s.state]:'Статус не подтверждён')}</small></div><h3 class="lc-session-title">${esc(s.title||'Название не передано')}</h3><dl class="wk-metadata">${row('Источник',(s.hash?'Торрент · ':'')+(s.source||'Не передан'))}${row('Способ',({'browser':'Встроенный · браузер','browser-hls':'Встроенный · HLS','native':'Встроенный · система устройства','external':'Внешнее приложение'})[s.method]||'Не определён')}${row('Позиция',seconds(s.position)+' / '+seconds(s.duration))}${row('Буфер плеера',s.fresh&&s.buffer!=null?s.buffer+' с':'Недоступно')}${row('Скорость раздачи',s.fresh&&s.downloadSpeed!=null?bytes(s.downloadSpeed)+'/с':'Недоступно')}${row('Последний сигнал Lampa',s.device?date(s.updated):'Не получен')}${s.streamActive?row('Проверка потока',date(s.streamChecked)):''}${row('Последняя ошибка',playbackErrors[s.error]||'Не зарегистрирована')}</dl>${!s.fresh||s.state==='ended'?`<div class="wk-actions"><button class="ghost" data-remove-session="${index}">Удалить запись</button></div>`:''}</div></article>`).join('')||empty('Активных просмотров нет.')}</div>`;
    panes.playback.querySelector('[data-clear-sessions]').onclick=async()=>{if(await operation('/playback',{action:'clear-inactive'},'Неактивные записи удалены'))refresh(true);};
