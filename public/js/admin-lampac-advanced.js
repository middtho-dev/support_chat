...
window.mountLampacAdvanced = function ({container, request, operation, toggle, publicUrl}) {
  ...
  function preferences(fields, values, snapshot, global = false, shared = {}) {
    ...
    return [...families].map(([family,entries],i)=>`<details class="lc-pref-family" ${i===0?'open':''}><summary>${esc(family)} <small>${entries.reduce((n,e)=>n+e[1].length,0)}</small></summary><div class="lc-pref-family-body">${entries.map(renderGroup).join('')}</div></details>`).join('');
  }
