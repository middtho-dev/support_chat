'use strict';
window.LampacPanel=(()=>{
  const root=$('lampac');let timer,loaded=false,pending=false,generation=0,advancedPanel;
  ...
  async function operation(path,body,message){if(pending)return;pending=true;root.querySelectorAll('button').forEach(b=>b.disabled=true);const g=generation;try{await request(path,body);if(g===generation)toast(message,'ok');return true;}catch(e){if(g===generation)toast(e.message,'err');}finally{pending=false;if(g===generation){root.querySelectorAll('button').forEach(b=>b.disabled=false);await refresh();}}}
  function render(d){
  ...
  async function refresh(){if(pending)return;const g=generation;try{const d=await request();if(g!==generation)return;if(!loaded){render(d);loaded=true;}else{state(d);$('lc-error').textContent='';}advancedPanel?.refresh();}catch(e){if(g===generation){if(!loaded){root.innerHTML='<div class="section"><h2>Lampac</h2><p id="lc-error" role="status"></p></div>';} $('lc-error').textContent=e.message;}}}
  function pause(){clearInterval(timer);timer=null;}
  function reset(){pause();advancedPanel?.destroy();advancedPanel=null;generation++;loaded=false;root.textContent='';}
