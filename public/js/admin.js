...
const IS_TG_MINI = !!TG || PAGE_PARAMS.get('tg') === '1';
...
function renderChatHeader() {
  ...
  $('cv-sub').textContent = `#${t.id.slice(0, 8)} · ${fmtDate(new Date(t.created_at))} · ${t.status === 'open' ? 'открыто' : 'закрыто'} · оператор: ${assignee}`;
  const btn = $('cv-toggle');
