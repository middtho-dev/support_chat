...
**Требования:** VPS с Ubuntu/Debian и публичным IPv4, домен с A-записью на этот IPv4, порты 80 и 443 открыты. AAAA-запись для домена не требуется.
...
Скрипт спросит домен, токен и Group ID, определит публичный IPv4 и покажет текущую DNS A-запись. Затем он установит Docker и Caddy, привяжет Caddy и приложение к IPv4, настроит HTTPS и запустит приложение.
