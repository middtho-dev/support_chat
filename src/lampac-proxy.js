...
function createLampacProxy({authorize,url=process.env.LAMPAC_SERVICE_URL||'http://127.0.0.1:7600',token=process.env.LAMPAC_SERVICE_TOKEN,fetcher=fetch}={}){
  return async(req,res)=>{
    ...
    if(!(req.method==='GET'&&['','/torrserver','/advanced','/torrents','/clients'].includes(suffix)||req.method==='POST'&&['/configure','/action','/torrserver','/advanced','/client','/torrents','/clients'].includes(suffix)))return res.sendStatus(404);
